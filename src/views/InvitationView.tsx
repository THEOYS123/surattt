  ...
  return (
    <div className="relative min-h-screen bg-stone-100 font-sans text-stone-900 selection:bg-amber-800 selection:text-white">
      ...
      <main className="max-w-xl mx-auto bg-white shadow-2xl min-h-screen pb-20 border-x border-stone-200">
        ...
        {/* ========================================================================= */}
        {/* SUSUNAN ACARA / RUNDOWN ACARA (MANUAL USER INPUT & PRESETS) */}
        {/* ========================================================================= */}
        {((d.rundown && d.rundown.length > 0) || (d.rundownItems && d.rundownItems.length > 0) || (isSchool && d.agendaRundown && d.agendaRundown.length > 0)) && (
          <section className="py-12 px-6 border-b border-stone-100 space-y-6" id="section-susunan-acara">
            <div className="text-center space-y-1">
              <span className="text-xs uppercase tracking-widest text-amber-700 font-semibold flex items-center justify-center gap-1.5">
                <Clock className="w-3.5 h-3.5" />
                <span>Susunan Acara</span>
              </span>
              <h3 className="font-serif-display text-2xl font-bold text-stone-900">
                Rangkaian & Jadwal Kegiatan
              </h3>
              <p className="text-xs text-stone-500 max-w-sm mx-auto">
                Susunan waktu dan agenda kegiatan yang telah dipersiapkan
              </p>
            </div>

            {/* If regular rundown exists */}
            {((d.rundown && d.rundown.length > 0) || (d.rundownItems && d.rundownItems.length > 0)) && (
              <div className="space-y-3 max-w-md mx-auto">
                {(d.rundown || d.rundownItems || []).map((item, idx) => (
                  <div
                    key={item.id || idx}
                    className="flex items-start gap-3.5 p-4 rounded-2xl bg-stone-50/80 border border-stone-200/80 hover:border-amber-300 transition-all shadow-2xs"
                  >
                    <div className="shrink-0 text-center">
                      <div className="px-2.5 py-1 bg-amber-100 text-amber-900 rounded-lg text-xs font-mono font-bold">
                        {item.time}
                      </div>
                    ...
                    <div className="space-y-1 flex-1 min-w-0">
                      ...
                      {item.location && (
                        <div className="flex items-center gap-1 text-[11px] font-medium text-amber-700 pt-0.5">
            ...
            {/* School agenda fallback if no general rundown */}
            {(!d.rundown || d.rundown.length === 0) && (!d.rundownItems || d.rundownItems.length === 0) && isSchool && d.agendaRundown && (
              <div className="space-y-3 max-w-md mx-auto">
